...
public class Busqueda extends JFrame {
	...
	public Busqueda() {
		...
		txtBuscar.setText("Ingrese id o apellido");
		txtBuscar.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent e) {
				 if (txtBuscar.getText().equals("Ingrese id o apellido")) {
					 txtBuscar.setText("");
		...
		txtBuscar.setForeground(SystemColor.activeCaptionBorder);
		txtBuscar.setBackground(SystemColor.controlDkShadow);
		...
		lblNewLabel_4.setBounds(331, 62, 312, 42);
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(filaReservas >=0) {
					...
					int confirmar = JOptionPane.showConfirmDialog(null, "Esta seguro que desea borrar esta reserva?" );
					
					if(confirmar == JOptionPane.YES_OPTION) {
						String valor = tbReservas.getValueAt(filaReservas, 0).toString();
						reservasControl.Eliminar(Integer.valueOf(valor));
						JOptionPane.showMessageDialog(contentPane, "reserva eliminada exitosamente");
						limpiarTabla();
						mostrarTablaReservas();
						listaHuespedes();
					}
					...
					int confirmaH = JOptionPane.showConfirmDialog(null, "Desea eliminar a este huesped?");
					...
					if(confirmaH == JOptionPane.YES_OPTION) {
						...
						JOptionPane.showMessageDialog(contentPane, "Huesped eliminado exitosamente");
						limpiarTabla();
						listaHuespedes();
						mostrarTablaReservas(); 
					}
...
//metodo para mostrar la tabla de reservas
...
//este metodo se encarga de mostrar la tabla filtrando por el id de la reserva
...
//metodos de los huespedes
private List<Huespedes> mostrarHuespedes(){
	return this.huespedControl.mostrarHuesped();
}
//metodo para buscar huespedes
private List<Huespedes> buscarHuespedesApellido(){ //ex-id
	return this.huespedControl.buscarHuesped(txtBuscar.getText());
}
//metodo que muestra la lista de huespedes
...
//filtrado de huespedes por id
private void listaHuespedesApellido() { //ex listahuespedesID
	List<Huespedes> huespedes = buscarHuespedesApellido();
	//modeloHuesped.setRowCount(0);
...
//metodos generales
//metodo para actulizar los huespedes
private void actualizarHuespedes() {
	...
	}, ()-> JOptionPane.showInternalMessageDialog(this, "BIEN HECHO"));
}
...
//metodo que obtiene y transforma la informacion entregada por el front para actualizar las reservas
private void ActualizarReservas() {
	...
	.ifPresent(fila ->{
		...
		try {
			...
			System.out.println("error el el metodo de actualizacion");
			throw new RuntimeException(e);
		}
		this.reservaVista.clearValor();
		...
		if(tbReservas.getSelectedColumn() ==0) {
			JOptionPane.showMessageDialog(this, "Los id no se pueden editar");
		}else {
			this.reservasControl.actualizarReserva(dataE, dataS, valor, formaPago, id);
			JOptionPane.showMessageDialog(this, String.format("Reserva modificada exitosamente"));
		}
	});
}

//metodo que calcula el valor de la reserva en base a las fechas editadas dentro del cuadro de busqueda
